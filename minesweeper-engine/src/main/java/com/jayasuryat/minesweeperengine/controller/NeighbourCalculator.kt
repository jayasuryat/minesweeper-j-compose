...
internal class NeighbourCalculator {
    ...
    private val visitedCells: MutableList<MineCell.ValuedCell.EmptyCell> = mutableListOf()

    fun getAllNeighbouringValueCell(
        cell: MineCell.ValuedCell.EmptyCell,
        grid: Grid,
    ): List<RawCell.RevealedCell> {
        visitedCells.clear()
        val neighbours = cell.getX3EmptyCells(grid)
        visitedCells.clear()
        return neighbours
    }

    private fun MineCell.ValuedCell.EmptyCell.getX3EmptyCells(
        grid: Grid,
    ): List<RawCell.RevealedCell> {
